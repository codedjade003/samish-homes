...
// src/components/admin/RecentPropertiesTable.tsx
const properties = [
  {
  ...
  return (
    <div className="bg-white rounded-lg shadow mb-6">
      ...
      <button onClick={() => setModalOpen(true)} className="bg-primary text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700 flex items-center">
          <FaPlus className="fas fa-plus mr-2"></FaPlus> Add New Property
       </button>
      </div>
              ...
              <tr key={i}>
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                    ${prop.status === "Active" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}>
                    {prop.status}
                  </span>
                </td>
                <td className="px-6 py-4 text-sm text-gray-500">{prop.agent}</td>
                <td className="px-6 py-4 text-sm font-medium">
                  <a href="#" className="text-primary hover:text-indigo-900 mr-3"><i className="fas fa-edit"></i></a>
                  <a href="#" className="text-danger hover:text-red-900"><i className="fas fa-trash"></i></a>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
        <div className="text-sm text-gray-500">
          Showing <span className="font-medium">1</span> to <span className="font-medium">3</span> of <span className="font-medium">247</span> properties
        </div>
